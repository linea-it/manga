...
def plot_lines(wavelength,  stellar_flux, pseudo_continuum, parnames, rest_wl_lines, solution,label_lines=True,label_abs_lines=True):
    ...
    if label_abs_lines: # para marcar a posicao das linhas em absorcao. Isso poderia ser um liga e desliga no plot. 
       ...
       for i in range(0, len(abs_lines), 1):
            ypos=1.1*plt.gca().get_ylim()[0]  # to lendo de novo no for por que tive um erro se deixo fora...e nao entendi pq.
            plt.axvline(abs_lines[i],ls='-.',color='green')
            plt.text(abs_lines[i]+5,ypos,abs_linesname[i],rotation='vertical',color='green')
...
plot_lines(wavelength,  stellar_flux, pseudo_continuum, parnames, rest_wl, solution,label_lines=True,label_abs_lines=True)
